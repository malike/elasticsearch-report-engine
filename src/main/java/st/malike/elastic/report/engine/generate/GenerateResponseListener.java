...
public class GenerateResponseListener implements ActionListener<SearchResponse> {
  ...
  public void onResponse(SearchResponse searchResponse) {
    ...
    try {
      ...
      final File reportFile = generateData.getFormat()
          .generate(generateData.getMapData(), dataList, generateData.getTemplateLocation(),
      ...
      if (generateData.getReturnAs().equals(Generator.ReturnAs.BASE64)) {
        message.setData(
            generateData.getFormat().objectToBase64String(reportFile, generateData.getFormat()));
      } else {
  ...
  public void onFailure(Exception exception) {
    throw new ElasticsearchException("Failed to create a response.",
